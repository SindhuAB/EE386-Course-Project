...
% tab = input('Enter the Huff Table name (for decoding purpose): ', 's');
% tb = fopen(tab, 'w+');
% idd = fopen(idd, 'w', 'n', 'UTF-8');  % Open the file with UTF-8 encoding
% fst1 = '';
% fst2 = '';
% ed = 0;
% din = 0;
...
% fclose(tb);
% 
% % Converting 8 bit Binary to UTF-8 encoded ASCII character and storing the result in the specified file%
% disp('Converting binary Huffman codes to UTF-8 encoded ASCII characters......');
% nme = input('Enter the destination file name : ', 's');
% disp('Generating the compressed file..........');
% id = fopen(nme, 'w', 'n', 'UTF-8');  % Open the file with UTF-8 encoding
% 
% for i = 1:8:t
%     ck = t - i + 1;
%     if (ck > 8)
